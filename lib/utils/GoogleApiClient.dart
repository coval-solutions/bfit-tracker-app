...
class GoogleApiClient extends http.BaseClient {
  ...
  GoogleApiClient._create();

  static Future<GoogleApiClient> create({defaultHeaders}) async {
    var client = GoogleApiClient._create();
    if (defaultHeaders == null || !client.defaultHeaders.containsKey(HttpHeaders.authorizationHeader)) {
      String token = await UserRepository.getIdToken();
      client.defaultHeaders[HttpHeaders.authorizationHeader] = 'Bearer $token';
    }
